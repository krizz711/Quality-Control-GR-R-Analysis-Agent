...
@pytest.mark.asyncio
async def test_grr_study_creates_audit_event(async_client, auth_token, db_conn):
    token = auth_token
    ...
    r = await db_conn.fetchrow("SELECT * FROM audit_events WHERE event_type = $1 ORDER BY created_at DESC LIMIT 1", 'grr_study_run')
...
@pytest.mark.asyncio
async def test_auth_failure_creates_audit_event(async_client, db_conn):
    resp = await async_client.get("/api/v1/measurements")
    assert resp.status_code in (401, 403)
    r = await db_conn.fetchrow("SELECT * FROM audit_events WHERE event_type = $1 ORDER BY created_at DESC LIMIT 1", 'auth_failure')
    assert r is not None
...
@pytest.mark.asyncio
async def test_alert_sent_creates_audit_event(fake_redis, db_conn):
    ...
    r = await db_conn.fetchrow("SELECT * FROM audit_events WHERE event_type = $1 ORDER BY created_at DESC LIMIT 1", 'alert_sent')
    assert r is not None
...
@pytest.mark.asyncio
async def test_sql_injection_on_audit_filter(async_client, auth_token, db_conn):
    token = auth_token
    ...
    count = await db_conn.fetchval("SELECT COUNT(*) FROM audit_events")
    assert count is not None


@pytest.mark.asyncio
async def test_no_pii_or_secrets_in_audit_records(async_client, db_conn):
    ...
    r = await db_conn.fetchrow("SELECT * FROM audit_events ORDER BY created_at DESC LIMIT 1")
    assert r is not None
    joined = str(dict(r))
...
@pytest.mark.asyncio
async def test_audit_retention_policy_exists(db_conn):
    r = await db_conn.fetchrow("SELECT * FROM timescaledb_information.jobs WHERE hypertable_name = 'audit_events' AND proc_name = 'policy_retention'")
    assert r is not None
...
@pytest.mark.asyncio
async def test_audit_events_is_hypertable(db_conn):
    r = await db_conn.fetchrow("SELECT * FROM timescaledb_information.hypertables WHERE hypertable_name = 'audit_events'")
    assert r is not None
